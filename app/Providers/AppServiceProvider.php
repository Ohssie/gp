...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        {
            $clean = "DELETE FROM package_subscription WHERE (status = 'incomplete' OR status != 'waiting') AND username NOT IN(SELECT username FROM users WHERE role = 'admin') AND updated_at <";
            $clean .= " CASE ";
            $clean .= " WHEN DAYOFWEEK(updated_at) = 6 THEN DATE_SUB(NOW(), INTERVAL ";
            $clean .= (int) config('settings.delete_records_after') + 3;
            $clean .= " DAY) ";
            $clean .= " WHEN DAYOFWEEK(updated_at) = 7 THEN DATE_SUB(NOW(), INTERVAL ";
            $clean .= (int) config('settings.delete_records_after') + 2;
            $clean .= " DAY)";
            $clean .= "WHEN DAYOFWEEK(updated_at) = 1 THEN DATE_SUB(NOW(), INTERVAL ";
